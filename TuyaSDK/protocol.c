...
{
  {DPID_AIR_QUALITY_INDEX, DP_TYPE_ENUM},
  {DPID_TEMP_CURRENT, DP_TYPE_VALUE},
  {DPID_HUMIDITY_VALUE, DP_TYPE_VALUE},
  {DPID_CO2_VALUE, DP_TYPE_VALUE},
  {DPID_TVOC, DP_TYPE_VALUE},
  {DPID_LED_1, DP_TYPE_VALUE},
  {DPID_LED2, DP_TYPE_VALUE},
  {DPID_LED3, DP_TYPE_VALUE},
...
void all_data_update(void)
{
    ...
    mcu_dp_enum_update(DPID_AIR_QUALITY_INDEX,3); //枚举型数据上报;	当前空气质量指数
    mcu_dp_value_update(DPID_TEMP_CURRENT,111); //VALUE型数据上报;	当前温度
...
******************************************************************************/
/*****************************************************************************
函数名称 : dp_download_led_1_handle
功能描述 : 针对DPID_LED_1的处理函数
输入参数 : value:数据源数据
        : length:数据长度
...
/*****************************************************************************
函数名称 : dp_download_led2_handle
功能描述 : 针对DPID_LED2的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_led2_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long led2;
    
    led2 = mcu_get_dp_download_value(value,length);
    /*
    //VALUE type data processing
    ...
    */
    ...
    //There should be a report after processing the DP
    ret = mcu_dp_value_update(DPID_LED2,led2);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_led3_handle
功能描述 : 针对DPID_LED3的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_led3_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long led3;
    
    led3 = mcu_get_dp_download_value(value,length);
    /*
    //VALUE type data processing
    ...
    */
    ...
    //There should be a report after processing the DP
    ret = mcu_dp_value_update(DPID_LED3,led3);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_switch1_handle
功能描述 : 针对DPID_SWITCH1的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
...
/*****************************************************************************
函数名称 : dp_download_switch2_handle
功能描述 : 针对DPID_SWITCH2的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
...
/*****************************************************************************
函数名称 : dp_download_switch3_handle
功能描述 : 针对DPID_SWITCH3的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_switch3_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:off/1:on
    ...
    switch3 = mcu_get_dp_download_bool(value,length);
    if(switch3 == 0) {
        //bool off
    }else {
        //bool on
    ...
    //There should be a report after processing the DP
    ret = mcu_dp_bool_update(DPID_SWITCH3,switch3);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
    ...
    ***********************************/
    ...
    switch(dpid) {
        case DPID_LED_1:
            //灯1处理函数
            ret = dp_download_led_1_handle(value,length);
        break;
        case DPID_LED2:
            //灯2处理函数
            ret = dp_download_led2_handle(value,length);
        break;
        case DPID_LED3:
            //灯3处理函数
            ret = dp_download_led3_handle(value,length);
        break;
        case DPID_SWITCH1:
            //开关1处理函数
            ret = dp_download_switch1_handle(value,length);
        break;
        case DPID_SWITCH2:
            //开关2处理函数
            ret = dp_download_switch2_handle(value,length);
        break;
        case DPID_SWITCH3:
            //开关3处理函数
            ret = dp_download_switch3_handle(value,length);
        break;
